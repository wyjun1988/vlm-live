...
학습 가중치를 평가할 때는 --geometry cut3r 로 바꿔야 한다.
...
def main() -> int:
    ...
    for vi, (ds, scene) in enumerate(vids):
        ...
        for m in modes:
            ...
            sessions[m] = StreamingSession(live, sel, geom_stride=10**9, visual_mode=vmode,
            ...
            for d in qs:
                ...
                if pc is not None:
                    ...
                    with torch.no_grad(), live.injector.primed(pre["geometry"].embeds, live.visual_pos_mask(ids)):
                        out = base.generate(input_ids=ids, attention_mask=torch.ones_like(ids),
